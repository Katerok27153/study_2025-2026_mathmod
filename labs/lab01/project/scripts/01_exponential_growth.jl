...
# Отключаем ВСЁ отображение графиков
ENV["GKSwstype"] = "100"  # Отключаем GUI
gr(show = false)          # Отключаем показ для GR бэкенда
default(show = false)      # Глобально отключаем показ
default(fmt = :png)        # Формат для сохранения
...
# Построим график решения (без отображения)
plt = plot(sol, label="u(t)", xlabel="Время t", ylabel="Популяция u",
...
# Ничего не возвращаем (это самое важное!)
nothing
